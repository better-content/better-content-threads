...
public record ThreadDefinition(String id,String title,ThreadSuit suit,int order,ThreadAspect aspect,ResourceLocation art,
    ...
    public record Route(String id,String label,String type,String value){public Route{if(!id.matches("[a-z0-9_]{1,48}"))throw new IllegalArgumentException("invalid route id");bounded(label,64,"route label");if(!type.matches("[a-z0-9_]{1,32}")||value==null||value.length()>160)throw new IllegalArgumentException("invalid route signal");}}
    ...
    private static List<Route> routes(JsonArray array){if(array==null)return List.of();return java.util.stream.StreamSupport.stream(array.spliterator(),false).map(e->e.getAsJsonObject()).map(o->new Route(o.get("id").getAsString(),o.get("label").getAsString(),o.get("type").getAsString(),o.has("value")?o.get("value").getAsString():"")).toList();}
}
