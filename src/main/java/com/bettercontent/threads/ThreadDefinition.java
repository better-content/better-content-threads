...
public record ThreadDefinition(String id,String conceptId,String owner,String title,ThreadSuit suit,int order,ThreadAspect aspect,ResourceLocation art,
                               String rule,String prose,String invitation,String action,List<Route> revealRoutes,
    ...
    public ThreadDefinition{
        ...
        bounded(rule,MAX_RULE,"rule");int words=wordCount(rule);if(words<20||words>35)throw new IllegalArgumentException("thread rule must contain 20..35 words");
        bounded(prose,MAX_TEXT,"prose");bounded(invitation,160,"invitation");bounded(action,192,"action");if(revealRoutes.isEmpty()||completionRoutes.isEmpty())throw new IllegalArgumentException("live thread lacks routes");
    }
