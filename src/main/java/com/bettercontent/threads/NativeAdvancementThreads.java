...
public final class NativeAdvancementThreads {
    ...
    public static void earned(AdvancementEvent.AdvancementEarnEvent event) {
        ...
        switch (event.getAdvancement().getId().toString()) {
            ...
            case "create:water_wheel" -> reveal(player, "kinetic_source", "water_wheel");
            case "create:mechanical_pump_0" -> complete(player, "rivers_turn_work", "kinetic_complete", "mechanical_pump");
            case "create:train" -> reveal(player, "train_stations", "assembled_train");
            case "create:long_travel" -> complete(player, "rails_turn_distance", "train_ride", "long_travel");
