...
final class LoadingBriefs implements ResourceManagerReloadListener {
    ...
    public void onResourceManagerReload(ResourceManager resources) {
        try (var reader = new InputStreamReader(resources.getResourceOrThrow(MANIFEST).open(), StandardCharsets.UTF_8)) {
            ...
            if (!"bc.loading_briefs.v3".equals(root.get("schema").getAsString())) throw new IllegalArgumentException("invalid loading brief schema");
            ...
            for (var element : root.getAsJsonArray("briefs")) {
                ...
                if (!brief.relatedThread().isEmpty() && !ThreadArt.BY_ID.containsKey(brief.relatedThread())) {
                    throw new IllegalArgumentException("unknown related thread: " + brief.relatedThread());
                }
                parsed.add(brief);
            }
