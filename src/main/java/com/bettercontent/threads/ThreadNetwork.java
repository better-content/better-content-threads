...
public final class ThreadNetwork {
    ...
    private static Card card(ThreadDefinition d,ThreadPlayerState s){boolean known=s.known.contains(d.id()),active=s.active.contains(d.id());var doorway=d.doorway();return new Card(d.id(),d.conceptId(),d.title(),d.suit().id(),d.order(),d.aspect().id(),d.art().toString(),known,known&&s.unread.contains(d.id()),active,s.completed.contains(d.id()),known?d.rule():"",known?d.prose():"",active?d.invitation():"",active?d.action():"",doorway==null?"":doorway.type(),doorway==null?"":doorway.target(),s.completionCounts.getOrDefault(d.id(),0),s.firstGeneration.getOrDefault(d.id(),-1L),s.lastGeneration.getOrDefault(d.id(),-1L),s.routeSummary(d.id()));}
    ...
    public record Card(String id,String conceptId,String title,String suit,int order,String aspect,String art,boolean known,boolean unread,boolean active,boolean completed,String rule,String prose,String invitation,String action,String doorwayType,String doorwayTarget,int completionCount,long firstGeneration,long lastGeneration,String routeSummary){
        public Card{ThreadPacketValidation.id(id);if(!conceptId.matches("[a-z0-9_.]{3,80}"))throw new IllegalArgumentException("invalid thread concept");ThreadPacketValidation.title(title);ThreadSuit.parse(suit);if(order<1||order>13)throw new IllegalArgumentException("invalid thread order");ThreadAspect.parse(aspect);ThreadPacketValidation.resource(art,"art");if(rule.length()>ThreadDefinition.MAX_RULE||prose.length()>ThreadDefinition.MAX_TEXT||invitation.length()>160||action.length()>192||routeSummary.length()>512)throw new IllegalArgumentException("oversized thread card text");if(!doorwayType.isEmpty()&&!ThreadDefinition.Doorway.validType(doorwayType))throw new IllegalArgumentException("invalid doorway type");if(doorwayTarget.length()>128||completionCount<0||firstGeneration< -1||lastGeneration< -1)throw new IllegalArgumentException("invalid thread history");if(!known&&(!rule.isEmpty()||!prose.isEmpty()||!invitation.isEmpty()||!action.isEmpty()))throw new IllegalArgumentException("unknown card leaked teaching copy");}
        void encode(FriendlyByteBuf b){b.writeUtf(id,48);b.writeUtf(conceptId,80);b.writeUtf(title,64);b.writeUtf(suit,16);b.writeVarInt(order);b.writeUtf(aspect,16);b.writeUtf(art,128);b.writeBoolean(known);b.writeBoolean(unread);b.writeBoolean(active);b.writeBoolean(completed);b.writeUtf(rule,ThreadDefinition.MAX_RULE);b.writeUtf(prose,ThreadDefinition.MAX_TEXT);b.writeUtf(invitation,160);b.writeUtf(action,192);b.writeUtf(doorwayType,24);b.writeUtf(doorwayTarget,128);b.writeVarInt(completionCount);b.writeLong(firstGeneration);b.writeLong(lastGeneration);b.writeUtf(routeSummary,512);}
        static Card decode(FriendlyByteBuf b){return new Card(b.readUtf(48),b.readUtf(80),b.readUtf(64),b.readUtf(16),b.readVarInt(),b.readUtf(16),b.readUtf(128),b.readBoolean(),b.readBoolean(),b.readBoolean(),b.readBoolean(),b.readUtf(ThreadDefinition.MAX_RULE),b.readUtf(ThreadDefinition.MAX_TEXT),b.readUtf(160),b.readUtf(192),b.readUtf(24),b.readUtf(128),b.readVarInt(),b.readLong(),b.readLong(),b.readUtf(512));}
    }
    ...
    public record Action(String action,String thread){
        ...
        private static void handle(ServerPlayer player,Action action){if(player==null)return;if(action.action.equals("open")){sync(player,true,List.of());return;}if(action.action.equals("recipe_viewed")){String active=ThreadSignals.activeCorrelation(player,"recipes_obey_this_world");ThreadSignals.emit(player,"recipe_viewed","emi",active==null?player.getUUID()+":recipe:"+player.server.getTickCount():active);return;}if(!ThreadDefinitions.INSTANCE.contains(action.thread))return;var state=ThreadPlayerState.get(player);if(!state.known.contains(action.thread))return;if(action.action.equals("read")){if(state.markRead(action.thread)){state.save(player);sync(player,false,List.of());}return;}if(!action.action.equals("issue")||state.unread.contains(action.thread))return;long now=System.currentTimeMillis(),previous=lastIssue.getOrDefault(player.getUUID(),0L);if(now-previous<1000)return;lastIssue.put(player.getUUID(),now);var stack=ThreadFacsimileItem.create(action.thread,player);if(!player.getInventory().add(stack)){player.displayClientMessage(Component.literal("Make room for the facsimile first."),true);return;}player.containerMenu.broadcastChanges();}
    }
