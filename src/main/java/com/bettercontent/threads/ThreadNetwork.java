...
public final class ThreadNetwork {
    ...
    public record Card(String id,String conceptId,String title,String suit,int order,String aspect,String art,boolean known,boolean unread,boolean active,boolean completed,String rule,String action,String doorwayType,String doorwayTarget,int completionCount,long firstGeneration,long lastGeneration,String routeSummary){
        public Card{ThreadPacketValidation.id(id);if(!conceptId.matches("[a-z0-9_.]{3,80}"))throw new IllegalArgumentException("invalid thread concept");ThreadPacketValidation.title(title);ThreadSuit.parse(suit);if(order<1||order>13)throw new IllegalArgumentException("invalid thread order");ThreadAspect.parse(aspect);ThreadPacketValidation.resource(art,"art");if(rule.length()>ThreadDefinition.MAX_RULE||action.length()>192||routeSummary.length()>512)throw new IllegalArgumentException("oversized thread card text");if(!doorwayType.isEmpty()&&!ThreadDefinition.Doorway.validType(doorwayType))throw new IllegalArgumentException("invalid doorway type");if(doorwayTarget.length()>128||completionCount<0||firstGeneration< -1||lastGeneration< -1)throw new IllegalArgumentException("invalid thread history");if(!known&&(!rule.isEmpty()||!action.isEmpty()))throw new IllegalArgumentException("unknown card leaked teaching copy");}
        void encode(FriendlyByteBuf b){b.writeUtf(id,48);b.writeUtf(conceptId,80);b.writeUtf(title,64);b.writeUtf(suit,16);b.writeVarInt(order);b.writeUtf(aspect,16);b.writeUtf(art,128);b.writeBoolean(known);b.writeBoolean(unread);b.writeBoolean(active);b.writeBoolean(completed);b.writeUtf(rule,ThreadDefinition.MAX_RULE);b.writeUtf(action,192);b.writeUtf(doorwayType,24);b.writeUtf(doorwayTarget,128);b.writeVarInt(completionCount);b.writeLong(firstGeneration);b.writeLong(lastGeneration);b.writeUtf(routeSummary,512);}
        static Card decode(FriendlyByteBuf b){return new Card(b.readUtf(48),b.readUtf(80),b.readUtf(64),b.readUtf(16),b.readVarInt(),b.readUtf(16),b.readUtf(128),b.readBoolean(),b.readBoolean(),b.readBoolean(),b.readBoolean(),b.readUtf(ThreadDefinition.MAX_RULE),b.readUtf(192),b.readUtf(24),b.readUtf(128),b.readVarInt(),b.readLong(),b.readLong(),b.readUtf(512));}
    }
